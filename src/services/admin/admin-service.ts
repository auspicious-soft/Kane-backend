...
export const dashboardService = async (payload: any, res: any) => {
	...
	const totalTopLeaders = await usersModel.countDocuments({ isDeleted: false, topLeaderPrivacy: false });
	const topLeaders = await usersModel.find({ isDeleted: false, topLeaderPrivacy: false }).select("-password").sort({ totalPoints: -1 }).skip(offset).limit(limit);
...
const totalPointsRedeemed = pointsRedeemed.reduce(
